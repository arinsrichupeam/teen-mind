...
export default function UserDetailDrawer({
  ...
  const getProvinceName = (provinceId: string | number) => {
    const province = provinces.find(
      (p) => p.id === parseInt(provinceId.toString())
    );
  ...
  const getDistrictName = (districtId: string | number) => {
    const district = districts.find(
      (d) => d.id === parseInt(districtId.toString())
    );
  ...
  const getSubdistrictName = (subdistrictId: string | number) => {
    const subdistrict = subdistricts.find(
      (s) => s.id === parseInt(subdistrictId.toString())
    );
  ...
  return (
    ...
    >
      <DrawerContent>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-2">
                  <div>
                    ...
                    <span className="font-medium ml-2">
                      {
                        prefix.find(
                          (val) => val.key == user.prefixId.toString()
                        )?.label
                      }{" "}
                    ...
                    user.address[0] ? (
                      <span className="font-medium ml-2">
                        {user.address[0].houseNo
                          ? `${user.address[0].houseNo} `
                          : ""}
                        {user.address[0].villageNo
                          ? `หมู่ที่ ${user.address[0].villageNo} `
                          : ""}
                        {user.address[0].soi
                          ? `ซอย ${user.address[0].soi} `
                          : ""}
                        {user.address[0].road
                          ? `ถนน ${user.address[0].road} `
                          : ""}
                        {user.address[0].subdistrict
                          ? `ตำบล${getSubdistrictName(user.address[0].subdistrict)} `
                          : ""}
                        {user.address[0].district
                          ? `อำเภอ${getDistrictName(user.address[0].district)} `
                          : ""}
                        {user.address[0].province
                          ? `จังหวัด${getProvinceName(user.address[0].province)}`
                          : ""}
                      </span>
                    ) : (
                  ...
                  <div className="md:col-span-2">
                    ...
                    user.emergency[0] ? (
                      <>
                        <span className="font-medium ml-2">
                          {user.emergency[0].name || "-"}
                        </span>
                        <span className="text-default-500 ml-4">โทรศัพท์:</span>
                        <span className="font-medium ml-2">
                          {user.emergency[0].tel || "-"}
                        </span>
                        <span className="text-default-500 ml-4">
                          ความสัมพันธ์:
                        </span>
                        <span className="font-medium ml-2">
                          {user.emergency[0].relation || "-"}
                        </span>
                      </>
                    ) : (
                      <span className="font-medium ml-2">-</span>
              ...
              <div className="flex justify-between items-center mb-3">
                ...
                {mode === "edit" && (
                  <Button
                    ...
                    size="md"
                    startContent={
                      <PlusIcon className="size-6 text-success-500" />
                    }
                    variant="flat"
                    onClick={() => {
        ...
        <DrawerFooter>
          <Button color="danger" size="md" variant="flat" onClick={onClose}>
            ปิด
          </Button>
        </DrawerFooter>
      </DrawerContent>
