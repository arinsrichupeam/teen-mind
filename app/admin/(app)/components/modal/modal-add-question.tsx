...
export const ModalAddQuestion = ({
    ...
    const isFormComplete = () => {
        const requiredFields = [
            'q2_q1', 'q2_q2',
            'phqa_q1', 'phqa_q2', 'phqa_q3', 'phqa_q4', 'phqa_q5', 
            'phqa_q6', 'phqa_q7', 'phqa_q8', 'phqa_q9',
            'addon_q1', 'addon_q2'
        ];
    ...
    const isStepComplete = (step: number) => {
        switch (step) {
            ...
            case 2:
                return formData.phqa_q1 !== "" && formData.phqa_q2 !== "" && 
                       formData.phqa_q3 !== "" && formData.phqa_q4 !== "" && 
                       formData.phqa_q5 !== "" && formData.phqa_q6 !== "" && 
                       formData.phqa_q7 !== "" && formData.phqa_q8 !== "" && 
                       formData.phqa_q9 !== "";
            case 3:
    ...
    const handleSubmit = async () => {
        ...
        for (let i = 1; i <= 9; i++) {
            const value = parseInt(formData[`phqa_q${i}` as keyof typeof formData] as string);
            if (value < 0 || value > 3) {
        ...
        if (parseInt(formData.addon_q1) !== 0 && parseInt(formData.addon_q1) !== 1) {
            alert("ค่า Addon ข้อที่ 1 ไม่ถูกต้อง (ต้องเป็น 0 หรือ 1)");
            return;
        }
        if (parseInt(formData.addon_q2) !== 0 && parseInt(formData.addon_q2) !== 1) {
        ...
        try {
            ...
            if (response.ok) {
                ...
                alert(`เกิดข้อผิดพลาด: ${errorData.error || "ไม่สามารถเพิ่มแบบทดสอบได้"}`);
            }
        } catch (error) {
    ...
    const renderStepContent = () => {
        switch (currentStep) {
                                ...
                                <Card key={index} className="p-4">
                                    <CardBody>
                                        <p className="text-sm mb-3">
                                            <span className="text-red-500 font-bold">*</span> {question}
                                        </p>
                                        <RadioGroup
                                            value={formData[`q2_q${index + 1}` as keyof typeof formData] as string}
                                            onValueChange={(value) =>
                                                setFormData(prev => ({
                                                    ...prev,
                                                    [`q2_q${index + 1}`]: value,
            ...
            case 2:
                return (
                    <div className="space-y-4">
                        <h3 className="text-lg font-semibold text-primary">แบบทดสอบ PHQA</h3>
                        <div className="space-y-4">
                            {qPhqa.map((question, index) => (
                                <Card key={index} className="p-4">
                                    <CardBody>
                                        <p className="text-sm mb-3">
                                            <span className="text-red-500 font-bold">*</span> <span className="font-semibold text-primary">{index + 1}. </span>
                                            {question}
                                        </p>
                                        <RadioGroup
                                            value={formData[`phqa_q${index + 1}` as keyof typeof formData] as string}
                                            onValueChange={(value) =>
                                                setFormData(prev => ({
                                                    ...prev,
            ...
            case 3:
                return (
                    <div className="space-y-4">
                        <h3 className="text-lg font-semibold text-primary">แบบทดสอบ Addon</h3>
                        <div className="space-y-4">
                            {phqaAddon.map((question, index) => (
                                <Card key={index} className="p-4">
                                    <CardBody>
                                        ...
                                        <RadioGroup
                                            value={formData[`addon_q${index + 1}` as keyof typeof formData] as string}
                                            onValueChange={(value) =>
                                                setFormData(prev => ({
                                                    ...prev,
                        ...
                        <ModalHeader className="flex flex-col gap-1">
                            <div className="flex justify-between items-center">
                                ...
                                <div className="flex gap-2">
                                    <span className={`px-2 py-1 rounded text-xs ${currentStep >= 1 ? 'bg-primary text-white' : 'bg-default-200'}`}>2Q</span>
                                    <span className={`px-2 py-1 rounded text-xs ${currentStep >= 2 ? 'bg-primary text-white' : 'bg-default-200'}`}>PHQA</span>
                                    <span className={`px-2 py-1 rounded text-xs ${currentStep >= 3 ? 'bg-primary text-white' : 'bg-default-200'}`}>Addon</span>
                                </div>
                            </div>
                        </ModalHeader>
                        <ModalBody>
