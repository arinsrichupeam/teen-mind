...
export async function POST(req: Request) {
  try {
    ...
    if (!citizenId || citizenId.length !== 13) {
      return NextResponse.json({ error: "กรอกเลขบัตรประชาชนไม่ครบถ้วน" }, { status: 400 });
    }

    const isDigit = /^[0-9]*$/.test(citizenId);
    if (!isDigit) {
      return NextResponse.json({ error: "เลขบัตรประชาชนต้องเป็นตัวเลขเท่านั้น" }, { status: 400 });
    }
    ...
    if (checksum !== parseInt(citizenId.charAt(12))) {
      return NextResponse.json({ error: "กรอกเลขบัตรประชาชนไม่ถูกต้อง" }, { status: 400 });
    }
    ...
    if (source === "user") {
      ...
      if (existingUser) {
        return NextResponse.json({ error: "เลขบัตรประชาชนนี้มีอยู่ในระบบแล้ว" }, { status: 400 });
      }
    ...
    return NextResponse.json({ error: "เกิดข้อผิดพลาดในการตรวจสอบเลขบัตรประชาชน" }, { status: 500 });
  }
} 